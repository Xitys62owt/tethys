...
trait JsonObjectWriter[A] extends JsonWriter[A] { self: JsonObjectWriter[A]^ =>
  ...
  def ++(that: JsonObjectWriter[A]^): JsonObjectWriter[A]^{this, that} =
    concat(that)

  def concat(
      that: JsonObjectWriter[A]^
  ): JsonObjectWriter[A]^{this, that} =
  ...
  override def contramap[B](
      fun: B => A
  ): JsonObjectWriter[B]^{this, fun} =
