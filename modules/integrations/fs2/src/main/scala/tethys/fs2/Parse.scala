...
object Parse {
  ...
  final case class OneDocument(path: List[String]) {
    ...
    def everyElementAs[T](using
        decoder: JsonReader[T]^
    ): OneDocumentDecoderApplied[T]^{decoder} =
      new OneDocumentDecoderApplied(path, decoder)
  }
...
private final class ArrayElementsParser[A](
  ...
  private var targetArrayDepths: List[Int] = Nil
  private var hasSeenTokens: Boolean = false
